...
1. Parse IPv6 pool config (already Phase 2) and expose `ApplyIPv6PDR` logic: accept IPv6 PDNType, populate PFCP PDR/FAR structs with IPv6 fields, add dual-stack conflict checks.
...
4. When kernel features are present, send IPv6 attrs in `installPDR`/`installFAR`, and route RA requests via new netlink op. Add `WNC` logs for enqueue success/failure.
...
1. **UAPI additions**
   ...
   - Extend user/kernel compat versioning; bump minor ABI and expose via `GTP5G_ATTR_VERSION`. Document the requirement in `README.md`.
